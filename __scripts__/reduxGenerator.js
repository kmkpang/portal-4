...
let reducerTemplateNoArgument = `export const {fncName} = (state:SeamlessImmutable.Immutable<any>) => {
  return state.merge(
    {
      {fetching}:{trueFalse}
    });
...
let reducerTemplateOneArgument = `export const {fncName} = (state:SeamlessImmutable.Immutable<any>,action:{fncName}Action) => {
  return state.merge(
    {
        {fetching}:{trueFalse},
        {arg1}:action.{arg1},
...
let reducerTemplateTwoArgument = `export const {fncName} = (state:SeamlessImmutable.Immutable<any>,action:{fncName}Action) => {
  return state.merge(
    {
        {fetching}:{trueFalse},
        {arg1}:action.{arg1},
...
let sagaTemplate = `export function * {fncName}(action:{fncName}Action) {
  const tag = '[{screamingFncName}]';
  try {
    // always wait for startup to finish first
    ...
    if (inputValid) {
        ...
        yield put(Actions.{sucFncName}(result));
    } else {
...
let sagaIndexTemplate = 'effects.takeLatest(ReduxTypes.{screamingFncName}, {fncName}),';
