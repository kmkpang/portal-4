...
class TwigEngine implements EngineInterface
{
    ...
    protected function getImageFunction($data)
    {
        return new Twig_SimpleFunction('image', function ($src, $alt = '', $cls = '', $attr = '', $force = false) use ($data) {
            ...
            if (strpos($src, 'data:', 0) === 0) {
                $ifApplied = true;
                $src = $src;
            } elseif (strpos($src, 'images', 0) === 0) {
                $ifApplied = true;
                $src = \JURI::root() . $src;
            } elseif (strpos($src, 'libraries', 0) === 0) {
                $ifApplied = true;
                $src = \JURI::root() . '/' . $src;
            } else {
                $src = ltrim($src, '/');
            ...
            if (!$ifApplied) {
                ...
                if ($isImageOptimized and filter_var($userWantOptimization, FILTER_VALIDATE_BOOLEAN)) {
                    ...
                    if ($this->optimizedImagesExists($src) and $readyToGo) {
                        ...
                        return new \Twig_Markup("<img src='{$srcurl}' srcset='{$srcset}' alt='{$alt}' class='{$cls}' {$attr} data-qx-img/>", 'UTF-8');
                    }
                }
    ...
    {
        return new Twig_SimpleFunction('validateJoomlaCaptcha', function ($value) use ($data) {
    ...
    protected function getJoomlaModuleFunction($data)
    {
        return new Twig_SimpleFunction('getJoomlaModule', function ($id, $style = 'raw') use ($data) {
            ...
            $params = ['style' => (isset($mparams->style) ? $mparams->style : $style)];
