...
class QuixControllerCollection extends JControllerForm
{
    ...
    public function save($key = null, $urlVar = 'id')
    {
        ...
        if ($result) {
            ...
            $view = base64_encode(JRoute::_('index.php?option=com_quix&view=collection&id=' . (int) $id));
    ...
    protected function cleanup($type, $item)
    {
        ...
        array_map('unlink', glob(get_compiled_assets_path() . "**/{$type}-{$item->id}-{$title}-{$item->builder}-*.*"));
    }
